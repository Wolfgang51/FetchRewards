...
class FetchRewardsApplicationTests {
	...
	public void entireFlowTest() throws Exception {
		...
		ResponseEntity<String> spendPointsResponse = this.restTemplate.postForEntity(baseUri + "/consumer/points", request,
				String.class);

		Assertions.assertEquals(spendPointsResponse.getStatusCode(), HttpStatus.OK);
		Assertions.assertTrue(spendPointsResponse.getBody().contains("{\"payer\":\"UNILEVER\",\"points\":-200}"));
		Assertions.assertTrue(spendPointsResponse.getBody().contains("{\"payer\":\"MILLER COORS\",\"points\":-4700}"));
		Assertions.assertTrue(spendPointsResponse.getBody().contains("{\"payer\":\"DANNON\",\"points\":-100}"));
		...
		Assertions.assertTrue(balanceResponse.getBody().contains("{\"payer\":\"DANNON\",\"points\":1000}"));
		Assertions.assertTrue(balanceResponse.getBody().contains("{\"payer\":\"UNILEVER\",\"points\":0}"));
		Assertions.assertTrue(balanceResponse.getBody().contains("{\"payer\":\"MILLER COORS\",\"points\":5300}"));
	}
}
